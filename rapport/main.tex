...
\begin{lstlisting}[language=C, caption={C donut}, label={code:donut}]
                 k;double sin()
         ,cos();main(){float A=
...
A compiler avec la commande \verb|gcc -o donut donut.c -lm|. Nécessite un terminal ANSI ou VT100 pour fonctionner. Ce code produit un donut qui tourne en 3D, le tout affiché via des caractères ASCII à la manière de pixels. 

Le programme Babble, par Sean Barrett (1992) :
...
Ce programme est franchement difficile à comprendre. C'est un programme mathématique pour faire des calculs de factorielles ou de plus grand facteur commun (\acrshort{pgcd}) entre deux nombres. \say{babble is a translator from a pseudo-assembly language into a subset of C suitable for execution on Charles Babbage's Analytical Engine.} \cite{SBarrett-Babble}.
